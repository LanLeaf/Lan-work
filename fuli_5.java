...
public class fuli_5 extends JFrame {
    ...
    */
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                fuli_5 frame = new fuli_5();
                frame.setDefaultCloseOperation(fuli_5.EXIT_ON_CLOSE);
	...
	private void jButton2MouseMouseClicked(MouseEvent event) {
		jTextArea0.setText("项目名\t投入本金\t利息\t年限\t现值\n");
		for(i=0;i<3;i++){
			for(j=0;j<5;j++){
				if(num[i][j]==null)
					jTextArea0.append(" "+"\t");
				else
					jTextArea0.append(num[i][j]+"\t");
			}
		jTextArea0.append("\n");
		}
	}
